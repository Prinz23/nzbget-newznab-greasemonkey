...
// @exclude        /^https?://(www\.)?usenet\-crawler\.com.*/
// @include        /^https?://(www\.)?[^/]+\.[^/]+/books|(browse|details\/[^/]+\/[^/]+|movies(\/browse|\/[^/]+)?|music|console|search\/[^/]+(\/)?|(myshows|mymovies)\/browse(aired)?(\?ob=&.*)?|series\/\d+(#S\d+|#Posted)?)(\?(t=\d{4,5}|offset=\d+)(&.*)?)?$/
// @run-at         document-end
// ==/UserScript==
